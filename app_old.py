...
def calculate_pnl(data):
    ...
    print(data)
    # Agrégation des comptes de classe 6 et 7
    class_6_7 = data[data['CompteNum'].str.startswith(('6', '7'))]
    class_6_7_agg = class_6_7.groupby([class_6_7['CompteNum'].str[:1]]).sum(numeric_only=True).reset_index()
    class_6_7_agg["Solde"]=class_6_7_agg["Debit"]-class_6_7_agg["Credit"]

    # Agrégation des comptes de classe à 2 chiffres
    class_2_digits = data[data['CompteNum'].str.startswith(('6', '7'))].groupby([class_6_7['CompteNum'].str[:2],"CompteLib_y"]).sum(numeric_only=True).reset_index()
    class_2_digits["Solde"]=class_2_digits["Debit"]-class_2_digits["Credit"]
    class_2_digits["CompteNum1"]=data['CompteNum'].str[:1]
    
    dfAgg=pd.concat([class_6_7_agg,class_2_digits])
    dfAgg["Solde"]=dfAgg["Debit"]=dfAgg["Credit"]
    print(dfAgg.columns)
    
    return dfAgg[["CompteNum","CompteLib_y","Solde"]].sort_values("CompteNum"),dfAgg.sort_values("CompteNum")
...
def results(filename):
    ...
    return render_template(
        ...
        class_6_7_agg=class_6_7_agg.to_html(classes='table table-striped', table_id="class_6_7_agg_table"),
        class_2_digits_agg=class_2_digits_agg.to_html(classes='table table-striped', table_id="class_2_digits_agg_table"),
